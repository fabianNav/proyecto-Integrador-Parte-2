...
export function pagar() {
    ...
    if (array.length > 0) {
        ...
        Swal.fire({
            icon: 'error',
            title: 'No elegiste ningún producto',
    ...
    productosLS.forEach(function (producto) {
        ...
        div.innerHTML = `
                <div class="col-4 mb-1">
                    ...
                    <a data-id=${producto.id} class="btn-sm me-1 mb-2 borrar-producto-compra fa-solid fa-trash-can"></a>
                    
                    </div>
                    <div class="col-2">
                    <input type="number" min="1" class="form-control text-center cantidad cantidad-carr" placeholder="Cantidad" value="${producto.cantidad}" >
                    <p class="text-center mt-4"> <strong>$</strong>
    ...
    if (e.target.classList.contains('cantidad')) {
        ...
        productosLS.forEach(function (productosLs, index) {
            if (productosLs.id === id) {
                ...
                precio.textContent = total.toFixed(0)
            }
        })
        localStorage.setItem('productos', JSON.stringify(productosLS))
        calcularTotal()
...
export function calcularTotal() {
    ...
    //console.log(total)
    impuestos = parseFloat(total * 0.18).toFixed(0)
    subtotal = parseFloat(total - impuestos).toFixed(0)

    document.querySelector('.total-resumen').textContent = total.toFixed(0)
